...
const generateTagsFromText = async function (text) {
...
const insertTagsIntoInput = function (tags) {
...
const onGenerate = async function () {
    ...
    if (!text) {
        alert('No post content found to generate tags from.');
        return;
    }

    const summary = await generateTagsFromText(text);
    if (!summary) {
        alert('Could not generate tags from the post content.');
        return;
    }
    ...
    insertTagsIntoInput(tags);
};

const createBtnElement = function () {
...
export { createBtnElement, onGenerate };